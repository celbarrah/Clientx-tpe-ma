...
export default function LogosMarquee() {
  ...
  return (
    <section className="bg-white border-y border-gray-100 py-8 overflow-hidden">
      <p className="text-center text-xs font-medium uppercase tracking-widest text-gray-300 mb-6">
        Choisie par les marques les plus exigeantes
      </p>
      <div className="relative">
        <div className="absolute left-0 top-0 bottom-0 w-20 bg-gradient-to-r from-white to-transparent z-10 pointer-events-none" />
        <div className="absolute right-0 top-0 bottom-0 w-20 bg-gradient-to-l from-white to-transparent z-10 pointer-events-none" />
        <div className="marquee-inner items-center">
          {doubled.map((logo, i) => (
            <div key={`${logo.alt}-${i}`} className="shrink-0 px-8 flex items-center justify-center">
              <Image
                ...
                width={100}
                height={40}
                className="h-7 w-auto object-contain grayscale opacity-40 hover:grayscale-0 hover:opacity-75 transition-all duration-300"
                unoptimized
              />
            </div>
          ))}
